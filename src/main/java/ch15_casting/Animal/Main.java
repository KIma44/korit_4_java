...
public class Main {
    ...
    // 결과값 :
        ...
        System.out.println("여기 앞에 있는 코드들이 업캐스팅 관련 수업입니다.");
        System.out.println("여기 뒤에 있는 코드들이 다운캐스팅 관련 수업할겁니다");
        /*
            업캐스팅의 경우에는 암시적으로 이루어집니다. 그래서 A a = new B(); 보고 대충 업캐스팅
...
//        animal2.fetch();
    Dog dog2 = (Dog) animal2;     // 다운캐스팅 방법
        dog2.makeSound();
        dog2.fetch();
((Dog) animal2).fetch();    //61번 라인이 있기 때문에 자동완성으로 바로 다운캐스팅이 이루어집니다.
...
//        dog3.makeSound();
        ...
        if (animal4 instanceof  Dog) {  // 애가 true라면 animal4 객체는 Animal animal4 = new Dog();로 생성됨
